 ...
 *
 * @date 2012-3-8
 * @author Joey come.a.mail(at)gmail.com
 * @link http://weibo.com/gongchengjun
 * @encoding UTF-8
 */

error_reporting(E_ALL & ~E_NOTICE);
if (! function_exists ( 'curl_init' )) {
    throw new Exception ( 'Curl for php is required.' );
    exit;
}

/**
 * require session_start(), 获取到access_token以后，保存到session中
 * 
 * @example
 * $kp = new Kuaipan('your consumer_key','your consumer_secret');
 * //跳转到授权页面，获取授权
 * $kp->getAuthorizationUri ('your callback url');
 * //callback 获取到request token以后，发起请求获取access token
 * //获取到access token以后，access会保存到session中
 * $access_token = $kp->getAccessToken ( 'oauth_token get by callback', 'oauth_verifier get by callback' );
 * access token : 
 *     array (
 *         'oauth_token' => 'xxxxxx',
 *         'oauth_secret' => 'xxxxxx',
...
class Kuaipan {
     ...
     */
    public function api($api, $path = '', $params = array (), $http_method = 'GET', $file_path = '') {
        $token = $this->getAccessToken();
        if (empty ( $token ['oauth_token'] ) || empty ( $token ['oauth_token_secret'] )) {
            throw new Exception ( 'An oauth_token and oauth_token_secret is required' );
        }
