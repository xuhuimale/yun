...
{	
	...
	public function __construct($key)
	{
		if(!extension_loaded('mcrypt')){
			throw new \Kingsoft\Exception('The storage encrypter requires the MCrypt extension');
		} elseif(($length = mb_strlen($key, '8bit')) !== self::KEY_SIZE){
			throw new \Kingsoft\Exception('Expecting a ' .  self::KEY_SIZE . ' byte key, got ' . $length);
		} else {
	...
	public function decrypt($cipherText)
	{
		...
		if($token === false){ // Unserialize fails if $token is boolean false
			throw new \Kingsoft\Exception('Failed to unserialize token');
		} else {
			return $token;
		}
	}
