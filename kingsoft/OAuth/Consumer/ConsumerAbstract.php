...
* @subpackage Consumer
*/
namespace Kingsoft\OAuth\Consumer;
use \Kingsoft\API as API;

abstract class ConsumerAbstract
{
	// Kingsoft web endpoint
	const WEB_URL = 'https://www.Kingsoft.com/1/';
	...
	// OAuth flow urls
	const REQUEST_TOKEN_URL = 'https://openapi.kuaipan.cn/open/requestToken';
	const AUTHORISE_URL = 'https://www.kuaipan.cn/api.php?ac=open';
	const ACCESS_TOKEN_URL = 'https://openapi.kuaipan.cn/open/accessToken';
	...
	*/
	 ...
	 * The user will be redirected to Kingsoft' web endpoint
	...
	*/
	...
	public function getAccessToken()
	{
		...
		$response = $this->fetch('POST', ACCESS_TOKEN_URL, '');
		$token = $this->parseTokenString($response['body']);
	...
	public function setSignatureMethod($method)
	{
		...
		switch($method){
			...
			case 'HMAC-SHA1':
				...
				break;
			default:
				throw new \Kingsoft\Exception('Unsupported signature method ' . $method);
		}
	}
	...
	public function setOutFile($handle)
	{
		if(!is_resource($handle) || get_resource_type($handle) != 'stream'){
			throw new \Kingsoft\Exception('Outfile must be a stream resource');
		}
		$this->outFile = $handle;
	}
	...
	{
		if(!is_resource($handle) || get_resource_type($handle) != 'stream'){
			throw new \Kingsoft\Exception('Infile must be a stream resource');
		}
		fseek($handle, 0);
		$this->inFile = $handle;
	}
	...
	* Kingsoft returns tokens in the response parameters, and
