...
/**
 * Kingsoft API base class
 * @author Ben Tadiar <ben@handcraftedbyben.co.uk>
 * @link https://github.com/benthedesigner/Kingsoft
 * @link https://www.Kingsoft.com/developers
 * @link https://status.Kingsoft.com Kingsoft status
 * @todo Add Kingsoft Exception classes
 * @package Kingsoft
 */
namespace Kingsoft;
...
{
	// API Endpoints
	const API_URL     = 'http://openapi.kuaipan.cn/1/';
	const CONTENT_URL = 'http://api-content.dfs.kuaipan.cn/1/';
	 ...
	 * Either `kuaipan` or `app_folder` (preferred)
	 * @var null|string
	 */
	private $root;
	 ...
	 * @link https://www.Kingsoft.com/developers/reference/api
	 * @param OAuth\Consumer $OAuth
	 * @param string $root Kingsoft app access type
	 */
	public function __construct(OAuth\Consumer\ConsumerAbstract $OAuth, $root = 'app_folder')
	{
		$this->OAuth = $OAuth;
		$this->setRoot($root);
	}
	...
	*/
	...
	{
		if($root !== 'app_folder' && $root !== 'kuaipan'){
			throw new Exception("Expected a root of either 'kuaipan' or 'app_folder', got '$root'");
		} else {
			$this->root = $root;
		}
	}
	...
	{
		$response = $this->fetch('POST', self::API_URL, 'account_info');
		return $response;
	}
	
	/**
	 * Uploads a physical file from disk
	 * Kingsoft impose a 150MB limit to files uploaded via the API. If the file
	...
	public function putFile($file, $filename = false, $path = '', $overwrite = true)
	{
		if(file_exists($file)){
			if(filesize($file) <= 157286400){
				$call = 'upload_file/' . $this->root . '/' . $this->encodePath($path);
	...
	public function getFile($file, $outFile = false, $revision = null)
	{
		...
		return array(
			...
			'meta' => json_decode($response['headers']['x-Kingsoft-metadata']),
			'data' => $response['body'],
		);
	}
...
//	}
	...
	{
		$call = 'history/' . $this->root . '/' . $this->encodePath($file);
		$params = array(
...
//	}
	...
	public function thumbnails($file, $format = 'JPEG', $size = 'small')
	{
		...
		$call = 'thumbnail/' . $this->root . '/' . $this->encodePath($file);
	 ...
	 * Kingsoft by passing it in as the from_copy_ref parameter on /fileops/copy
